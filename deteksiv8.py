...
model_path = 'yolov8n.pt'  # Model YOLOv8 Nano (pretrained)
...
while True:
    ...
    # Deteksi objek menggunakan YOLOv8 nano dengan threshold confidence 0.7
    ...
    for result in results:
        if hasattr(result, 'boxes') and result.boxes is not None:
            for box, cls in zip(result.boxes.xyxy, result.boxes.cls):
                ...
                # Cek apakah class_id dalam daftar kelas
                if class_id in model.names:
                    class_name = model.names[class_id]  # Ambil nama kelas
                else:
                    class_name = "Unknown"
    ...
    cv2.imshow('Deteksi Sampah dengan YOLOv8 Nano', annotated_frame)
